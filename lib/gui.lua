...
function KC_G.settingsWindowGUISetup()
	...
	--START Kills Table
	local tlw = KillCounter_Settings_Window_Table
	tlw:ClearAnchors()
	tlw.DataOffset = 0
    tlw.MaxLines = 8
    tlw.MaxColumns = 2
	...
	tlw:SetHeight(320)
    tlw:SetWidth(500)
    tlw:SetAnchor(TOPLEFT,KillCounter_Settings_Window,TOPLEFT,50,86)
...
function KC_G.statsWindowGUISetup()
	...
	--START Kills Table
	local tlw = KillCounter_Stats_Window_Stats_Table
	...
	tlw:SetHeight(425)
    tlw:SetWidth(500)
    tlw:SetAnchor(TOPLEFT,KillCounter_Stats_Window,TOPLEFT,80,125)
    tlw:SetDrawLayer(DL_BACKGROUND)
    tlw:SetMouseEnabled(true)
    tlw:SetHandler("OnMouseWheel",function(self,delta)
    ...
    --START KILLS TABLE
    local tkw = KillCounter_Stats_Window_Kills_Table
	tkw:ClearAnchors()
	tkw.DataOffset = 0
    tkw.MaxLines = 16
    tkw.MaxColumns = 7
	...
	tkw:SetHeight(455)
    tkw:SetWidth(850)
    tkw:SetAnchor(TOPLEFT,KillCounter_Stats_Window,TOPLEFT,25,125)
    ...
    --START Kills Table
	local tsw = KillCounter_Stats_Window_Seige_Table
	--tlw.Logo = KillCounter_Stats_Window_Kills_Logo
	tsw:ClearAnchors()
	tsw.DataOffset = 0
    tsw.MaxLines = 5
	...
	tsw:SetHeight(150)
    tsw:SetWidth(500)
    tsw:SetAnchor(TOPLEFT,KillCounter_Stats_Window,TOPLEFT,80,125)
    ...
    local tdw = KillCounter_Stats_Window_Deaths_Table
	tdw:ClearAnchors()
	tdw.DataOffset = 0
    tdw.MaxLines = 16
    tdw.MaxColumns = 5
	...
	tdw:SetHeight(455)
    tdw:SetWidth(850)
    tdw:SetAnchor(TOPLEFT,KillCounter_Stats_Window,TOPLEFT,25,125)
	...
	local tbw = KillCounter_Stats_Window_Breakdown_Table
	--tlw.Logo = KillCounter_Stats_Window_Kills_Logo
	tbw:ClearAnchors()
	tbw.DataOffset = 0
    tbw.MaxLines = 15
	...
	tbw:SetHeight(455)
    tbw:SetWidth(500)
    tbw:SetAnchor(TOPLEFT,KillCounter_Stats_Window,TOPLEFT,80,125)
    ...
    --START Stats Breakdown
	local tbgw = KillCounter_Stats_Window_Breakdown_Graph
	--tlw.Logo = KillCounter_Stats_Window_Kills_Logo
	tbgw:ClearAnchors()
	tbgw.DataOffset = 0
    tbgw.MaxLines = 13
	...
	tbgw:SetHeight(138)
    tbgw:SetWidth(275)
    tbgw:SetAnchor(TOPLEFT,KillCounter_Stats_Window,TOPLEFT,600,125)
	...
	local tbdgw = KillCounter_Stats_Window_Breakdown_Death_Graph
	--tlw.Logo = KillCounter_Stats_Window_Kills_Logo
	tbdgw:ClearAnchors()
	tbdgw.DataOffset = 0
    tbdgw.MaxLines = 13
	...
	tbdgw:SetHeight(138)
    tbdgw:SetWidth(275)
    tbdgw:SetAnchor(TOPLEFT,KillCounter_Stats_Window,TOPLEFT,600,310)
...
function KC_G.Percentages()
	...
	local adp = ad/ totalKills
	local dcp = dc/totalKills
	local epp = ep/totalKills

	local tempp = templar/totalKills
	local dkp = dragonknight/totalKills
	local nbp = nightblade/totalKills
	local sorcp = sorceror/totalKills
...
function KC_G.Stats_Window_Switch(to)
	--first hide everything
	--overview
	stats_table:SetHidden(true)
	stats_table.Logo:SetHidden(true)

	kills_table:SetHidden(true)
	seige_table:SetHidden(true)
	deaths_table:SetHidden(true)
	breakdown_table:SetHidden(true)
	breakdown_graph:SetHidden(true)
	breakdown_death_graph:SetHidden(true)

	if to == "Seige" then
		--d("seige")
		seige_table:SetHidden(false)
		stats_table.Logo:SetHidden(false)
	elseif to == "Overview" then 
		--d("overview")
		stats_table:SetHidden(false)
		stats_table.Logo:SetHidden(false)
	elseif to == "Kills" then
		--d("kills")
		kills_table:SetHidden(false)
	elseif to == "Death" then
		deaths_table:SetHidden(false)
	elseif to == "Breakdown" then
		breakdown_table:SetHidden(false)
		breakdown_graph:SetHidden(false)
		breakdown_death_graph:SetHidden(false)
	else 
		d("Invalid Button Press")
	end
end
...
function KC_G.updateStats(override)
	...
	if (not showingStats or stats_window) and not override == nil then return end
	local l = 1
...
function KC_G.UpdateGui()
	...
	if showingStats and KC_G.savedVars ~= nil and stats_window ~= nil and stats_table ~= nil then 
	        	...
	        	if i==1 then
	        		...
	        		if i==2 then
			   		...
			   		if i==8 then
				        ...
				        if j==2 then stats_table.Lines[i].Columns[j]:SetText(kbs) end
		        		if j==3 then stats_table.Lines[i].Columns[j]:SetText(KC_G.GetKillingBlows()) end
			   		end
			    end
		    end
	    end
	    ...
	    breakdown_graph.DragonKnight.Label:SetText("|CabababDragon Knights: ".. KC_Fn.round(dkp*100, 2).."% " .. dragonknight .. " Kills")
	    ...
	    breakdown_graph.Sorceror.Label:SetText("|CabababSorcerors: ".. KC_Fn.round(sorcp*100, 2).."% " .. sorceror .. " Kills")

	    breakdown_graph.NightBlade.Label:SetText("|CabababNightBlades: ".. KC_Fn.round(nbp*100, 2).."% " .. nightblade .. " Kills")
	    breakdown_graph.NightBlade:SetDimensions(((breakdown_graph:GetWidth()*.75)-10) * nbp,30)
	    
	    breakdown_graph.Templar.Label:SetText("|CabababTemplars: ".. KC_Fn.round(tempp*100, 2).."% " .. templar .. " Kills")
	    breakdown_graph.Templar:SetDimensions(((breakdown_graph:GetWidth()*.75)-10) * tempp,30)

	    local ad, dc, ep, adp, dcp, epp, templar, dragonknight, nightblade, sorceror, tempp, dkp, nbp, sorcp = KC_G.DeathPercentages()

	    breakdown_death_graph.DragonKnight.Label:SetText("|CabababDragon Knights: ".. KC_Fn.round(dkp*100, 2).."% " .. dragonknight .. " Deaths")
	    ...
	    breakdown_death_graph.Sorceror.Label:SetText("|CabababSorcerors: ".. KC_Fn.round(sorcp*100, 2).."% " .. sorceror .. " Deaths")

	    breakdown_death_graph.NightBlade.Label:SetText("|CabababNightBlades: ".. KC_Fn.round(nbp*100, 2).."% " .. nightblade .. " Deaths")
	    breakdown_death_graph.NightBlade:SetDimensions(((breakdown_graph:GetWidth()*.75)-10) * nbp,30)
	    
	    breakdown_death_graph.Templar.Label:SetText("|CabababTemplars: ".. KC_Fn.round(tempp*100, 2).."% " .. templar .. " Deaths")
